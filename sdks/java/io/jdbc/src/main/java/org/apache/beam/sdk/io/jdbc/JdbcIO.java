  ...
  public static class Write<T> extends PTransform<PCollection<T>, PDone> {
    ...
    public PDone expand(PCollection<T> input) {
      ...
      if (input.hasSchema() && !hasStatementAndSetter()) {
        ...
        inner = inner.withPreparedStatementSetter(new AutoGeneratedPreparedStatementSetter(fields, input.getToRowFunction()));
      }
    ...
    private class AutoGeneratedPreparedStatementSetter implements PreparedStatementSetter<T> {
      ...
      Map<Integer, PreparedStatementSetCaller> preparedStatementFieldSetterMap = new HashMap<>();

      AutoGeneratedPreparedStatementSetter(List<Schema.Field> fields, SerializableFunction<T, Row> toRowFn) {
        this.fields = fields;
      ...
      private void populatePreparedStatementFieldSetter() {
        IntStream.range(0, fields.size()).forEach((index) -> {
          Schema.FieldType fieldType = fields.get(index).getType();
          switch (fieldType.getTypeName()) {
            case BYTE: {
              preparedStatementFieldSetterMap.put(index,
                      (element, ps, i) -> ps.setByte(i + 1, element.getByte(i)));
              break;
            }
            case INT16: {
              preparedStatementFieldSetterMap.put(index,
                      (element, ps, i) -> ps.setInt(i + 1, element.getInt16(i)));
              break;
            }
            case INT64: {
              preparedStatementFieldSetterMap.put(index,
                      (element, ps, i) -> ps.setLong(i + 1, element.getInt64(i)));
              break;
            }
            case DECIMAL: {
              preparedStatementFieldSetterMap.put(index,
                      (element, ps, i) -> ps.setBigDecimal(i + 1, element.getDecimal(i)));
              break;
            }
            case FLOAT: {
              preparedStatementFieldSetterMap.put(index,
                      (element, ps, i) -> ps.setFloat(i + 1, element.getFloat(i)));
              break;
            }
            case DOUBLE: {
              preparedStatementFieldSetterMap.put(index,
                      (element, ps, i) -> ps.setDouble(i + 1, element.getDouble(i)));
              break;
            }
            case DATETIME: {
              preparedStatementFieldSetterMap.put(index,
                      (element, ps, i) -> ps.setTimestamp(i + 1, new Timestamp(element.getDateTime(i).getMillis())));
              break;
            }
            case BOOLEAN: {
              preparedStatementFieldSetterMap.put(index,
                      (element, ps, i) -> ps.setBoolean(i + 1, element.getBoolean(i)));
              break;
            }
            case BYTES: {
              preparedStatementFieldSetterMap.put(index,
                      (element, ps, i) -> ps.setBytes(i + 1, element.getBytes(i)));
              break;
            }
            case INT32: {
              preparedStatementFieldSetterMap.put(index,
                      (element, ps, i) -> ps.setInt(i + 1, element.getInt32(i)));
              break;
            }
            case STRING: {
              preparedStatementFieldSetterMap.put(index,
                      (element, ps, i) -> ps.setString(i + 1, element.getString(i)));
              break;
            }
            case ARRAY:
              preparedStatementFieldSetterMap.put(index,
            ...
            case LOGICAL_TYPE: {
              ...
              switch (jdbcType) {
                case DATE:
                  preparedStatementFieldSetterMap.put(index, (element, ps, i) -> {
                    ps.setDate(index + 1, new Date(element.getDateTime(index).getMillis()));
                  });
                  break;
                case TIME:
                  preparedStatementFieldSetterMap.put(index, (element, ps, i) -> {
                    ps.setTime(index + 1, new Time(element.getDateTime(index).getMillis()));
                  });
                  break;
                case TIMESTAMP_WITH_TIMEZONE:
                  preparedStatementFieldSetterMap.put(index, (element, ps, i) -> {
                    ps.setTimestamp(index + 1, new Timestamp(element.getDateTime(index).getMillis()));
                  });
                  break;
                default:
                  throw new RuntimeException("LOGICAL_TYPE of [" + jdbcType + "] is not supported");
              }
              break;
            }
            default:
      ...
      public void setParameters(T element, PreparedStatement preparedStatement) throws Exception {
        ...
        IntStream.range(0, fields.size()).forEach((index) -> {
          try {
            preparedStatementFieldSetterMap.get(index).set(row, preparedStatement, index);
          }
