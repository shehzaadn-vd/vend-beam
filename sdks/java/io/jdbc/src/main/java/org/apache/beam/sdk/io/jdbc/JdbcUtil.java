...
public class JdbcUtil {
      ...
      case LOGICAL_TYPE:
        {
          ...
          switch (jdbcType) {
            case DATE:
              return (element, ps, i, fieldWithIndex) -> {
                ps.setDate(
                    i + 1, getDateFromDateTime(element.getDateTime(fieldWithIndex.getIndex()).toDateTime()));
              };
            case TIME:
              return (element, ps, i, fieldWithIndex) -> {
                ps.setTime(
                    i + 1, getTimeFromDateTime(element.getDateTime(fieldWithIndex.getIndex()).toDateTime()));
              };
            case TIMESTAMP_WITH_TIMEZONE:
              return (element, ps, i, fieldWithIndex) -> {
                DateTime dateTime = element.getDateTime(fieldWithIndex.getIndex()).toDateTime();
                ps.setTimestamp(i + 1, new Timestamp(element.getDateTime(fieldWithIndex.getIndex()).getMillis()), Calendar.getInstance(TimeZone.getTimeZone(dateTime.getZone().getID())));
              };
            default:
  ...
  private static void validateLogicalTypeLength(Schema.Field field, Integer length) {
    try{
      if(!field.getType().getLogicalType().getArgument().isEmpty()) {
      ...
      // do nothing and proceed with the insertion
    }
  }

  private static Time getTimeFromDateTime(DateTime dateTime) {
