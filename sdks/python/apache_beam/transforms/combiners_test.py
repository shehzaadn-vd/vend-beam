...
class ApproximateQuantilesTest(unittest.TestCase):
  ...
  def test_quantiles_globaly_comparable(self):
    ...
    with TestPipeline() as p:
      data = range(101)
      pc = p | Create(data)
      quantiles = pc | combine.ApproximateQuantiles.Globally(5, reverse=True)
      assert_that(quantiles, equal_to([[100, 75, 50, 25, 0]]))
  ...
  def test_quantiles_per_key_descending_order(self):
    ...
    with TestPipeline() as p:
      data = self._kv_data
      pc = p | Create(data)
      quantiles = pc | combine.ApproximateQuantiles.PerKey(2, reverse=True)
      assert_that(quantiles, equal_to([('a', [3, 1]), ('b', [100, 1])]))
  ...
  def test_quantiles_per_key_with_key_argument(self):
    with TestPipeline() as p:
      ...
      pc = p | Create(data)
      quantiles = pc | combine.ApproximateQuantiles.PerKey(2, key=len)
  ...
  def test_alternate_comparator(self):
    ...
    with TestPipeline() as p:
      pc = p | Create(data)
      quantiles = pc | combine.ApproximateQuantiles.Globally(3, key=len)
      assert_that(quantiles, equal_to([["b", "aaa", "ccccc"]]))
  ...
  def test_global_display_data(self):
    ...
    aq = combine.ApproximateQuantiles.Globally(3, comparator, key=len,
                                               reverse=True)
    ...
    expected_items = [
        ...
        DisplayDataItemMatcher('key', aq.key.__class__),
        DisplayDataItemMatcher('reverse', aq.reverse.__class__)
    ]
    hc.assert_that(data.items, hc.contains_inanyorder(*expected_items))

  def test_perkey_display_data(self):
    comparator = lambda a, b: a - b  # order by length
    aq = combine.ApproximateQuantiles.PerKey(3, comparator, key=len,
                                             reverse=True)
    data = DisplayData.create_from(aq)
    expected_items = [
        DisplayDataItemMatcher('num_quantiles', aq.num_quantiles),
